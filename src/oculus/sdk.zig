...
pub fn init(b: *Builder, user_config: ?UserConfig, versions: ToolchainVersions) *Sdk {
    ...
    const host_tools = blk: {
        const zip_add = b.addExecutable("zip_add", sdkRoot() ++ "/kuba-zip/zip_add.zig");
        zip_add.addCSourceFile(sdkRoot() ++ "/kuba-zip/zip.c", &[_][]const u8{
            "-std=c99",
            "-fno-sanitize=undefined",
        });
        zip_add.addIncludeDir(sdkRoot() ++ "/kuba-zip");
        zip_add.linkLibC();
...
pub const CreateAppStep = struct {
    ...
    pub fn getAndroidPackage(self: @This(), name: []const u8) std.build.Pkg {
        return self.sdk.b.dupePkg(std.build.Pkg{
            .name = name,
            .path = .{ .path = sdkRoot() ++ "/android-support.zig" },
